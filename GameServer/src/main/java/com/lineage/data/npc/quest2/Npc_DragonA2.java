...
public class Npc_DragonA2 extends NpcExecutor {
	...
	public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
		try {
			...
			// 傳送前往<font fg=66ff66><var src="#0"></font>執行副本條件限制
	...
	private void staraQuestA(L1PcInstance pc) {
		try {
				...
				for (L1PcInstance otherPc : party.partyUsers().values()) {
					if (i <= (users - 1)) {
						if (otherPc.getId() != party.getLeaderID()) {
							...
							// 傳送成員
							L1Teleport.teleport(otherPc, 32957, 32743, (short) mapid, 1, true);
			...
			L1SpawnUtil.spawnDoor(quest, 10008, 7858, 32741, 32712, (short) mapid, 0);// A 32739 32712 1011 A /
			L1SpawnUtil.spawnDoor(quest, 10009, 7859, 32779, 32681, (short) mapid, 1);// B 32779 32681 1011 B \
			L1SpawnUtil.spawnDoor(quest, 10010, 7858, 32861, 32709, (short) mapid, 0);// C 32861 32709 1011 C /
			
			final L1Location loc = new L1Location(32951, 32842, mapid);
			L1SpawnUtil.spawn(71026, loc, 5, showId);// 新法利昂(1階段)
			...
			for (L1NpcInstance npc : quest.npcList()) {
				if (npc instanceof L1MonsterInstance) {
					...
					if (npc.getNpcId() == 71026) {// 新法利昂(1階段)
						continue;
					}
					if (npc.getNpcId() == 71027) {// 新法利昂(2階段)
						continue;
					}
					if (npc.getNpcId() == 71028) {// 新法利昂(3階段)
						continue;
					}
