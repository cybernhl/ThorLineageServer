...
public class Npc_AuctionBoard extends NpcExecutor {
	...
	public void action(final L1PcInstance pc, final L1NpcInstance npc, final String cmd, final long amount) {
		try {
			...
			if (temp[0].equalsIgnoreCase("select")) { // 查看資訊
				pc.sendPackets(new S_AuctionBoardRead(objid , temp[1]));
				
			} else if (temp[0].equalsIgnoreCase("map")) { // 觀看位置
				pc.sendPackets(new S_HouseMap(objid, temp[1]));
				
			} else if (temp[0].equalsIgnoreCase("apply")) { // 參加拍賣
				final L1Clan clan = WorldClan.get().getClan(pc.getClanname());
				if (clan != null) {
					if (pc.isCrown() && (pc.getId() == clan.getLeaderId())) { // 王族 血盟盟主
						if (pc.getLevel() >= 15) {
							if (clan.getHouseId() == 0) {
								pc.sendPackets(new S_ApplyAuction(objid, temp[1]));
							} else {
								pc.sendPackets(new S_ServerMessage(521)); // 已經擁有血盟小屋。
								isCloseList = true;
							}
						} else {
							pc.sendPackets(new S_ServerMessage(519)); // 等級15以下的王族無法參與拍賣。
							isCloseList = true;
						}
					} else {
