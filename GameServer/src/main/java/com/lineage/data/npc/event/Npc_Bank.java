...
public class Npc_Bank extends NpcExecutor {
	...
	private class BankType {
		String _account_name = null;// 帳戶名稱
		boolean _isNew = false;// false:舊客戶 true:新客戶
		String _pass_x1 = "";// 第一次輸入的密碼
		String _pass = "";// 輸入的密碼
		boolean _type = false;// false:存款 true:取款
		boolean _repass = false;// false:無 true:更改密碼
	}
	...
	public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
		try {
			pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "1y_bank_1", 
					new String[]{RangeLong.scount(BankSet.BANKMAX).toString(), RangeLong.scount(BankSet.BANK_INTEREST_OVER).toString()}));
	...
	public void action(final L1PcInstance pc, final L1NpcInstance npc, final String cmd, final long amount) {
		try {
			...
			if (cmd.equalsIgnoreCase("A")) {// 進入銀行作業
				if (bank == null) {
					...
					// 恢復預設質
					bankType._isNew = false;// false:舊客戶 true:新客戶
					bankType._pass_x1 = "";// 第一次輸入的密碼
					bankType._pass = "";// 輸入的密碼
					bankType._type = false;// false:存款 true:取款
					bankType._repass = false;// false:無 true:更改密碼
					
					pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "1y_bank_2", 
							new String[]{
						RangeLong.scount(bank.get_adena_count()).toString(), 
				...
				if (!bankType._type && bank != null) {
					if (bank.get_adena_count() >= BankSet.BANKMAX) {
						...
						final L1ItemInstance item = pc.getInventory().checkItemX(BankSet.BANK_ITEMID, amount);
						if (item != null) {
							long count = amount + bank.get_adena_count();
							pc.getInventory().removeItem(item, amount);// 刪除道具
							...
							isClose = true;
							// 找回物品資訊
							final L1Item itemtmp = ItemTable.get().getTemplate(BankSet.BANK_ITEMID);
			...
			} else if (cmd.equalsIgnoreCase("D")) {// 回到銀行作業
				_bankTypeMap.remove(pc.getAccountName());
				pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "1y_bank_1", 
						new String[]{RangeLong.scount(BankSet.BANKMAX).toString(), String.valueOf(BankSet.BANK_INTEREST_OVER)}));
				
			} else if (cmd.equalsIgnoreCase("E")) {// 重新輸入密碼
				bankType._pass = "";
				bankType._pass_x1 = "";
				pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "1y_bank_4", new String[]{""}));
				
			} else if (cmd.equalsIgnoreCase("F")) {// 更改取款密碼
				pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "1y_bank_8", new String[]{""}));
