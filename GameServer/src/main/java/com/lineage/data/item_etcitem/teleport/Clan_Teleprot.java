...
public class Clan_Teleprot extends ItemExecutor {
	...
	public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
		...
		if (pc.getClanid() != 0) {
			/*
			 * 不能傳送的地方 if ((pc.getX() >= 33472 && pc.getX() <= 33855) && (pc.getY() >= 32191 && pc.getY() <= 32460) && pc.getMapId() == 4 && !pc.getInventory().checkItem(70314) && !Config.Bookmark) { // \f1這個地點不能夠標記。 return; // 黃昏山脈 } else if ((pc.getX() >= 34225 && pc.getX() <= 34300) && (pc.getY() >= 33095 && pc.getY() <= 33280) && pc.getMapId() == 4 && !pc.getInventory().checkItem(70314) && !Config.Bookmark) { // \f1這個地點不能夠標記。 return; // 黃昏山脈 } else if ((pc.getX() >= 34200 && pc.getX() <= 34300) && (pc.getY() >= 33280 && pc.getY() <= 33515) && pc.getMapId() == 4 && !pc.getInventory().checkItem(70314) && !Config.Bookmark) { // \f1這個地點不能夠標記。 return; } else if ((L1CastleLocation.checkInAllWarArea(pc.getX(), pc.getY(), pc.getMapId()) || L1HouseLocation.isInHouse(pc.getX(), pc.getY(), pc.getMapId())) && !pc.isGm()) { // \f1這個地點不能夠標記。 return; }
			 */

//			if (!pc.getMap().isClan_Teleprot()) {
...
//			}
			...
			final String msg = pc.getName() + "使用" + item.getName();
...
//						}
						...
						listner.sendPackets(new S_Message_YN(620, "血盟成員" + pc.getName() + "使用了穿雲箭(血盟)需要您支援。(Y/N)！"));
					}
				}
			}
			World.get().broadcastPacketToAll(new S_ServerMessage("血盟:【" + clanName + "】玩家：【" + msg + "】位置在【" + mapName + "】使用了穿雲箭(血盟)千軍萬馬來相見。"));
			World.get().broadcastPacketToAll(new S_BlueMessage(0, "\\f2血盟:【" + clanName + "】玩家：【" + msg + "】位置在【" + mapName + "】使用了穿雲箭(血盟)千軍萬馬來相見。"));
			// 刪除道具
