...
public class Clan_Letter extends ItemExecutor {
	...
	private boolean writeClanLetter(final int itemId, final L1PcInstance pc,
		...
		if (targetClan == null) {
			pc.sendPackets(new S_ServerMessage(434)); // 沒有收信者。
			return false;
		}
	...
	private boolean sendLetter(final L1PcInstance pc, final String name,
		...
		if (target != null) {
			if (target.getInventory().checkAddItem(item, 1) == L1Inventory.OK) {
				...
				target.sendPackets(new S_ServerMessage(428)); // 您收到鴿子信差給你的信件。
			} else {
				if (isFailureMessage) {
					// 對方的負重太重，無法再給予。
			...
			if (CharacterTable.doesCharNameExist(name)) {
				...
				if (isFailureMessage) {
					pc.sendPackets(new S_ServerMessage(109, name)); // 沒有叫%0的人。
				}
				return false;
			}
		}
