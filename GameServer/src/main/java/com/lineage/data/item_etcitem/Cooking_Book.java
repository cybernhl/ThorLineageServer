...
public class Cooking_Book extends ItemExecutor {
	...
	private void makeCooking(final L1PcInstance pc, final int cookNo) {
		...
		if (!isNearFire) {
			pc.sendPackets(new S_ServerMessage(1160)); // 為了料理所以需要柴火。
			return;
		}
		if (pc.getMaxWeight() <= pc.getInventory().getWeight()) {
			pc.sendPackets(new S_ServerMessage(1103)); // 身上無法再攜帶烹飪的物品
			return;
		}
		...
		if (is6394) {
			pc.sendPackets(new S_ServerMessage(1101)); // 烹飪失敗...。
			pc.broadcastPacketX10(new S_SkillSound(pc.getId(), 6394));
		}
		
		if (isErr) {
			pc.sendPackets(new S_ServerMessage(1102)); // 不夠材料可以烹飪...
		}
	}
}
