...
public class AttachUnkAbility2 extends ItemExecutor {
    public static ItemExecutor get() {
        return new AttachUnkAbility2();
    }
    ...
    public void execute(int[] data, L1PcInstance pc, L1ItemInstance item) {
        ...
        if (rand(0, 999999) < this.chance) {
            ...
            World.get().broadcastPacketToAll(new S_BlueMessage(0, "\\fX" + pc.getName() + " 的 " + tgItem.getViewName() + " 賦予未鑑定成功了！"));
            World.get().broadcastPacketToAll(new S_BlueMessage(0, "\\f=" + pc.getName() + " 的 " + tgItem.getViewName() + " 賦予未鑑定成功了！"));
        } else {
            pc.getInventory().removeItem(item, 1);
            pc.getInventory().updateItem(item, L1PcInventory.COL_ENCHANTLVL);
