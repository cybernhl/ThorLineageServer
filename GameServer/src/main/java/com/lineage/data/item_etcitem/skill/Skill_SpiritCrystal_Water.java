...
public class Skill_SpiritCrystal_Water extends ItemExecutor {
	...
	public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
		...
		if (!pc.isElf()) {
			...
			if (nameId.equalsIgnoreCase("精靈水晶(水之元氣)")) {// 精靈水晶(水之元氣)
				// 技能編號
				skillid = WATER_LIFE;
				// 分組
				magicLv = 13;

			} else if (nameId.equalsIgnoreCase("精靈水晶(生命之泉)")) {// 精靈水晶(生命之泉)
				// 技能編號
				skillid = NATURES_TOUCH;
				// 分組
				magicLv = 14;

			} else if (nameId.equalsIgnoreCase("精靈水晶(水之防護)")) {// 精靈水晶(水之防護)
				// 技能編號
				skillid = AQUA_PROTECTER;
				// 分組
				magicLv = 14;

			} else if (nameId.equalsIgnoreCase("精靈水晶(生命的祝福)")) {// 精靈水晶(生命的祝福)
				// 技能編號
				skillid = NATURES_BLESSING;
				// 分組
				magicLv = 15;

			} else if (nameId.equalsIgnoreCase("精靈水晶(生命呼喚)")) {// 精靈水晶(生命呼喚)
				// 技能編號
				skillid = CALL_OF_NATURE;
				// 分組
				magicLv = 15;
				
			} else if (nameId.equalsIgnoreCase("精靈水晶(污濁之水)")) {// 精靈水晶(污濁之水)
				// 技能編號
