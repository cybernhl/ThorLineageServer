...
public class SpawnTable {
	...
	public void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				// 刪除遺失地圖怪物刷新
				int mapids = rs.getShort("mapid");
				final MapData mapidtem = MapsTable.get().getTemplate(mapids);
				if (mapidtem == null) {// 刪除遺失地圖怪物刷新
					
						deleteMap(mapids);
					_log.error("不存在的map編號: " + mapids + " 刪除刷新資料!");
				}
				// 刪除遺失地圖怪物刷新 end
	...
	public static void deleteMap(final int mapid) {
		...
		try {
			cn = DatabaseFactory.get().getConnection();
			ps = cn.prepareStatement(
					"DELETE FROM `spawnlist` WHERE `mapid`=?");
			ps.setInt(1, mapid);
			ps.execute();
