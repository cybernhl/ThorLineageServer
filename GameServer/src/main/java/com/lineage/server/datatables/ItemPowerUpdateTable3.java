 ...
 *
 */
public class ItemPowerUpdateTable3 {

	private static final Log _log = LogFactory.getLog(ItemPowerUpdateTable3.class);

	private static Map<Integer, L1ItemPowerUpdate> _updateMap = 
			new HashMap<Integer, L1ItemPowerUpdate>();
	
	private static ItemPowerUpdateTable3 _instance;
	private final Lock lock = new ReentrantLock(false);

	public static ItemPowerUpdateTable3 get() {
		if (_instance == null) {
			_instance = new ItemPowerUpdateTable3();
		}
		return _instance;
	}
	...
	public void load() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM `server_item_power_update_3`");
	...
	public static void delete(final int itemid) {
		...
		try {
			...
			ps = cn.prepareStatement(
					"DELETE FROM `server_item_power_update_3` WHERE `itemid`=?");
			ps.setInt(1, itemid);
			ps.execute();
