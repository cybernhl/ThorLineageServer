...
public class L1Search implements L1CommandExecutor {
	...
	private void find_object(L1PcInstance pc, String name) {
		try {
			...
			statement.close();
//祝福
			statement = con.prepareStatement("SELECT item_id,name,bless FROM armor_bless WHERE name Like '%" + name + "%'");
			int count6 = 0;
			rs = statement.executeQuery();
			while(rs.next()) {
				if (count6 == 0) {
					pc.sendPackets(new S_SystemMessage("祝福防具:"));
				}
				str1 = rs.getString(1);
				str2 = rs.getString(2);
				bless = rs.getInt(3);
			...
			statement.close();
			//祝吳武器
			statement = con.prepareStatement("SELECT item_id,name,bless FROM weapon_bless WHERE name Like '%" + name + "%'");
			int count7 = 0;
			rs = statement.executeQuery();
			while(rs.next()) {
				if (count7 == 0) {
					pc.sendPackets(new S_SystemMessage("祝福武器:"));
				}
				str1 = rs.getString(1);
				str2 = rs.getString(2);
				bless = rs.getInt(3);
