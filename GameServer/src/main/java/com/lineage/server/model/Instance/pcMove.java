...
public class pcMove extends NpcMoveExecutor {
	...
	public void setDirectionMove(final int dir) {
		if (_pc.hasSkillEffect(L1SkillId.STATUS_FREEZE)) { //被冻时不能移动 QQ:759347094
			return;
		}
	...
	public int moveDirection(int x, int y) {
		...
		try {
			// 取回與目標點距離
			final double d = _pc.getLocation().getLineDistance(new Point(x, y));
			if (d > 20D) { // 距離超過30 (追蹤停止)
				return -1;
...
//			final int x = _pc.getX(); 
//			final int y = _pc.getY(); 
//			if (!_pc.getMap().isPassable(x, y, h, null)) {
//				return -1;
//			}
//		}
		return h;
    ...
    private int _serchCource(final int x, final int y) // 目标点Ｘ 目标点Ｙ
    {
        ...
        for (i = 0; i < 8; i++) {
            ...
            if (serchMap[locNext[0]][locNext[1]]) {
                ...
                switch (i) {
                    case 0:
                        found = _pc.getMap().isPassable(tmpX, tmpY + 1, i,_pc);
                        break;

                    case 1:
                        found = _pc.getMap().isPassable(tmpX - 1, tmpY + 1, i,_pc);
                        break;

                    case 2:
                        found = _pc.getMap().isPassable(tmpX - 1, tmpY, i,_pc);
                        break;
