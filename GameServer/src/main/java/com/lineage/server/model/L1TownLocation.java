...
public class L1TownLocation {
	...
	public static int[] getGetBackLoc(final int town_id) { // town_id歸還先座標返
		...
		case TOWNID_TALKING_ISLAND: // TI
			...
			loc[0] = GETBACK_LOC_TALKING_ISLAND[2].getX();
			loc[1] = GETBACK_LOC_TALKING_ISLAND[2].getY();
		...
		case TOWNID_SILENT_CAVERN: // 沈默洞窟
		...
		case TOWNID_RECLUSE_VILLAGE: // 隱裡
	...
	public static int getTownTaxRateByNpcid(final int npcid) { // npcid町稅率返
		int tax_rate = 0;

		final int town_id = getTownIdByNpcid(npcid);
		if ((town_id >= 1) && (town_id <= 10)) {
			final L1Town town = TownReading.get().getTownTable(town_id);
			tax_rate = town.get_tax_rate() + 2; // 2%固定稅
		}
		return tax_rate;
	}

	public static int getTownIdByNpcid(final int npcid) { // npcidtown_id返
		// 城：王國全域
		// 城：、
		// 城：、、
		// 城：、話島
		...
		// XXX:NPCL1CastleLocation持狀態（未整理）
		int town_id = 0;
