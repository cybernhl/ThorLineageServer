...
public class L1GuardInstance extends L1NpcInstance {
	...
	public void receiveDamage(L1Character attacker, final int damage) { // 攻擊ＨＰ減使用
		...
		if ((this.getCurrentHp() > 0) && !this.isDead()) {
			if (damage >= 0) {
				/*if (!(attacker instanceof L1EffectInstance)) { // FW
	...
	private boolean checkHasCastle(final L1PcInstance pc, final int castleId) {
		...
		if (!isExistDefenseClan) { // 城主居
			return true;
		}

		if (pc.getClanid() != 0) { // 所屬中
