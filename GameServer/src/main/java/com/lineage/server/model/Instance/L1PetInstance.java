...
public class L1PetInstance extends L1NpcInstance {
		...
		default:
			if ((this._petMaster != null) && (this._petMaster.getMapId() == this.getMapId())) { // ●
				...
				if (this.getLocation().getTileLineDistance(_petMaster.getLocation()) > 2) {
					int tmpX = getHeading() < 4 ? index : 0;
					int tmpY = getHeading() >= 4 ? index : 0;
					if (_npcMove != null) {
						int dir = _npcMove.moveDirection(_petMaster.getX() + tmpX, _petMaster.getY() + tmpY);
						if (dir == -1) { // 主人離休憩狀態
							_checkMove++;
							if (_checkMove >= 10) {
								// 控制者遺失
	...
	public L1PetInstance(L1Npc template, L1PcInstance master) {
		...
		this._itemObjId = -1;
		this._type = null;
		this.index = this.getPetList().size();
	...
	public L1PetInstance(final L1Npc template, final L1PcInstance master, final L1Pet pet) {
		...
		this.index = master.getPetList().size();
...
//				setSkillEffect(43, 1800000);
	...
	public void receiveDamage(final L1Character attacker, final int damage) {
		...
		if (this.getCurrentHp() > 0) {
			if (damage > 0) { // 回復場合攻擊。
				this.setHate(attacker, 0); // 無
	...
	private long deathPenalty() {
		...
		long exp = 0;
		exp = (long) (needExp * 0.15);
		if (exp == 0) {
			return 0;
		}
		CalcExp.addExpPet(this, -exp, 0);
		if (oldLevel > this.getLevel()) {
			...
			int hp = hpUpRange.randomValue();
			if ((this.getMaxHp() - hp) <= 0) {
				hp = 0;
			}
			int mp = mpUpRange.randomValue();
			if ((this.getMaxMp() - mp) <= 0) {
				mp = 0;
			}
			this.addMaxHp(-hp);
			this.addMaxMp(-mp);
	...
	public synchronized void death(final L1Character lastAttacker) {
		if (!this.isDead()) {
			if (lastAttacker instanceof L1PcInstance) {
				final L1PcInstance pc = (L1PcInstance) lastAttacker;
				int lawful = pc.getLawful();
				lawful += this.getLawful() * -1;
				pc.setLawful(lawful);
				pc.sendPackets(new S_Lawful(pc));
				this.setLawful(0);
			}
			this.setDead(true);
			this.setLawful(0);
    ...
    public void call_New() {
        if (this._type != null) {
            final int id = this._type.getMessageId(L1PetType.getMessageNumber(this.getLevel()));
            if (id != 0) {
                this.broadcastPacketX10(new S_NpcChat(this, "$" + id));
            }
        }
      
        // 移動至主人身邊休息
        this.setCurrentPetStatus(1);
    }
	/**
	 * 設置目標
	 * @param target
	...
	public void onPerceive(final L1PcInstance perceivedFrom) {
		try {
			...
			perceivedFrom.sendPackets(new S_NPCPack_Pet(this, perceivedFrom)); // 系認識
			if (getMaster() != null) {
	...
	public void onAction(final L1PcInstance player) {
		...
		if (!ConfigOther.ATTACK_PET && master.equals(player)) {// 攻擊者是主人
	...
	public void onItemUse() {
		if (!this.isActived()) {
			this.useItem(USEITEM_HASTE, 100); // １００％確率使用
		}
		if (this.getCurrentHp() * 100 / this.getMaxHp() < 40) { // ＨＰ４０％
			this.useItem(USEITEM_HEAL, 100); // １００％確率回復使用
		}
	}
