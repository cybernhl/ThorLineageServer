...
public class HASTE extends SkillMode {
	...
	public int start(final L1PcInstance srcpc, final L1Character cha, final L1Magic magic, final int integer) throws Exception {
		...
		if (cha.getMoveSpeed() != 2) { // 減速以外
			...
			int time = cha.getSkillEffectTimeSec(L1SkillId.HASTE);
			if (time <= 0) {
				time = 0;
			}
			time += integer;
			...
			cha.setSkillEffect(L1SkillId.HASTE, time * 1000);
			cha.broadcastPacketAll(new S_SkillHaste(cha.getId(), 1, time));
			cha.setMoveSpeed(1);
