...
public class L1WarSpawn {
	...
	public void spawnTower(final int castleId) {
		...
		final L1Npc l1npc = NpcTable.get().getTemplate(npcId); // 取回npc資料
		int[] loc = new int[3];
	...
	private L1NpcInstance spawnWarObject(final int npcid, final int locx, final int locy, final int mapid, final int h) {
		try {
