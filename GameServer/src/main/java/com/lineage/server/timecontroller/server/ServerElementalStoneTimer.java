...
public class ServerElementalStoneTimer extends TimerTask {
	...
	private static final int MAX_COUNT = ConfigAlt.ELEMENTAL_STONE_AMOUNT; // 設置個數
	private static final int INTERVAL = 3; // 設置間隔 秒
	private static final int SLEEP_TIME = 30; // 設置終了後、再設置時間 秒
	...
	public void run() {
		try {
			...
			while (this._itemList.size() < MAX_COUNT) { // 減場合
				final L1Location loc = new L1Location(this.nextPoint(), this._map);

				if (!this.canPut(loc)) {
					// XXX 設置範圍內全PC居場合無限…
					continue;
				}

				this.putElementalStone(loc);

				Thread.sleep(INTERVAL * 1000); // 一定時間每設置
			}
