...
public class C_Door extends ClientBasePacket {
	...
	public C_Door(final byte[] decrypt, final ClientExecutor client) {
		try {
			// 資料載入
			this.read(decrypt);
			...
			if (obj instanceof L1DoorInstance) {
				...
				if (((door.getDoorId() >= 5001) && (door.getDoorId() <= 5010))) { // 水晶洞窟
					return;
				}
				...
				case 10001:// 殭屍鑰匙
					...
					if (pc.getInventory().consumeItem(40594, 1)) { // 殭屍鑰匙
						door.open();
					}
					break;
