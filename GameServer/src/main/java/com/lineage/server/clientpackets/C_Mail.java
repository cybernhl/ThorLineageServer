...
public class C_Mail extends ClientBasePacket {
	...
	public C_Mail(final byte[] decrypt, final ClientExecutor client) {
		try {
			// 資料載入
			this.read(decrypt);
			...
			case 0x20: // 一般信件寄出
				if (pc != null) {
					...
					clientPackD(pc, type, receiverName, textr, client);
				}
				break;
				...
				if (clan != null) {
					for (final String name : clan.getAllMembers()) {
						...
						if (clanPc != null) { // 中
							...
							if (!mails.isEmpty()) {
								clanPc.sendPackets(new S_Mail(mails, TYPE_NORMAL_MAIL));
							}
	...
	private void clientPackD(L1PcInstance pc, int type, String receiverName, byte[] textr, ClientExecutor client) {
		try {
			...
			if (pc.getInventory().consumeItem(40308, 50)) {
				if (receiver != null) { // 連線
					...
					try {
						final L1PcInstance restorePc = CharacterTable.get().restoreCharacter(receiverName, client);
						if (restorePc != null) {
							if (MailReading.get().getMailSizeByReceiver(receiverName, TYPE_NORMAL_MAIL) >= 20) {
								pc.sendPackets(new S_Mail(type));
								return;
