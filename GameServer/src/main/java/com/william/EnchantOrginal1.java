 ...
 * 裝備強化能力系統
 */
public class EnchantOrginal1 {

	private static Logger _log = Logger.getLogger(EnchantOrginal1.class
			.getName());

	private static final Log _logx = LogFactory.getLog(EnchantOrginal1.class
			.getName());

	private static EnchantOrginal1 _instance;

	private final HashMap<Integer, L1WilliamEnchantOrginal1> _ArmorIndex = new HashMap<Integer, L1WilliamEnchantOrginal1>();

	public static EnchantOrginal1 getInstance() {
		if (_instance == null) {
			_instance = new EnchantOrginal1();
		}
		return _instance;
	}

	private EnchantOrginal1() {
		loadArmorOrginal();
	}
	...
	public void loadArmorOrginal() {
		...
		try {
			...
			pstm = con
					.prepareStatement("select * from 系統_裝備強化加成 order by 強化值,裝備類型 asc");
			rs = pstm.executeQuery();
			fillWeaponSkill(rs);

		} catch (final SQLException e) {
			_log.log(Level.SEVERE, "error while creating 系統_裝備強化加成 table", e);
