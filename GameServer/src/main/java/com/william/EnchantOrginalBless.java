 ...
 * 裝備強化能力系統
 */
public class EnchantOrginalBless {

	private static Logger _log = Logger.getLogger(EnchantOrginalBless.class
			.getName());

	private static final Log _logx = LogFactory.getLog(EnchantOrginalBless.class
			.getName());

	private static EnchantOrginalBless _instance;

	private final HashMap<Integer, L1WilliamEnchantOrginal> _ArmorIndex = new HashMap<Integer, L1WilliamEnchantOrginal>();

	public static EnchantOrginalBless getInstance() {
		if (_instance == null) {
			_instance = new EnchantOrginalBless();
		}
		return _instance;
	}

	private EnchantOrginalBless() {
		loadArmorOrginal();
	}
	...
	public void loadArmorOrginal() {
		...
		try {
			...
			pstm = con
					.prepareStatement("select * from 系統_祝福武器強化加成 order by 強化值,武器類型 asc");
			rs = pstm.executeQuery();
			fillWeaponSkill(rs);

		} catch (final SQLException e) {
			_log.log(Level.SEVERE, "error while creating 系統_祝福武器強化加成  table", e);
